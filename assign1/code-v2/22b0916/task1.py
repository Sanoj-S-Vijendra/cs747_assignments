...
class UCB(Algorithm):
    ...
    def give_pull(self):
        ...
        for a in range(self.num_arms):
            if self.counts[a] == 0:
                return a
        ucb_values = self.values+np.sqrt((self.c*np.log(self.total_counts))/(self.counts))
...
class KL_UCB(Algorithm):
    ...
    def give_pull(self):
        ...
        for a in range(self.num_arms):
            ...
            while(high-low>1e-6):
                mid=(low+high)/2.0
                if(kl_div(self.values[a],mid)>(math.log(self.total_counts)+self.c*math.log(max(math.log(max(self.total_counts,2)),1.0000000001)))/self.counts[a]):
                    high = mid
                else:
                    low = mid
...
class Thompson_Sampling(Algorithm):
    ...
    def give_pull(self):
        ...
        self.total_counts+=1
        x = [np.random.beta((self.success[a]+1),(self.failure[a]+1)) for a in range(self.num_arms)]
